...
export class AddDocs1746554350105 implements MigrationInterface {
    name = 'AddDocs1746554350105'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`asunto\` (\`id\` int NOT NULL AUTO_INCREMENT, \`titulo\` varchar(255) NOT NULL, \`estado\` enum ('enviado', 'proceso', 'entregado') NOT NULL, \`fecha_envio\` datetime NOT NULL, \`fecha_revision\` datetime NULL, \`fecha_entrega\` datetime NULL, \`id_asesoramiento\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`documento\` (\`id\` int NOT NULL AUTO_INCREMENT, \`nombre\` varchar(255) NOT NULL, \`ruta\` varchar(255) NOT NULL, \`subido_por\` enum ('cliente', 'asesor') NOT NULL, \`created_at\` datetime NOT NULL, \`id_asunto\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
