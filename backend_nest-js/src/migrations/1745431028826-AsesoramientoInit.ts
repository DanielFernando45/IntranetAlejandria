...
export class AsesoramientoInit1745431028826 implements MigrationInterface {
    name = 'AsesoramientoInit1745431028826'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        //await queryRunner.query(`CREATE TABLE \`asesoramiento\` (\`id\` int NOT NULL AUTO_INCREMENT, \`estado\` enum ('activo', 'desactivado', 'finalizado') NOT NULL, \`fecha_inicio\` datetime NOT NULL, \`fecha_fin\` datetime NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`asesoramiento\` (\`id\` int NOT NULL AUTO_INCREMENT, \`estado\` enum ('activo', 'desactivado', 'finalizado') NOT NULL, \`fecha_inicio\` datetime NOT NULL, \`fecha_fin\` datetime NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`procesos_asesoria\` ADD CONSTRAINT \`FK_94f8df2aa71583b6dfc0e744872\` FOREIGN KEY (\`id_cliente\`) REFERENCES \`cliente\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`procesos_asesoria\` ADD CONSTRAINT \`FK_51cbc8ccb27cc92c4e568ab0b1e\` FOREIGN KEY (\`id_asesor\`) REFERENCES \`asesor\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`procesos_asesoria\` ADD CONSTRAINT \`FK_asesoramiento_proceso\` FOREIGN KEY (\`id_asesoramiento\`) REFERENCES \`asesoramiento\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
